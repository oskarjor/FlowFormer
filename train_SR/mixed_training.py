...
from torchVAR.utils.paired_dataset import build_mixed_dataset
...
def get_next_batch(datalooper, synthetic_datalooper):
    if synthetic_datalooper is None:
        return next(datalooper)
    if random.random() < FLAGS.synthetic_ratio:
        return next(synthetic_datalooper)
    return next(datalooper)
...
def train(argv):
    ...
    elif FLAGS.dataset == "mixed":
        train_set, val_set, synthetic_dataset = build_mixed_dataset(
            input_data_path=FLAGS.input_data_path,
            real_data_path="./imagenet",
            pre_image_size=FLAGS.pre_image_size,
            post_image_size=FLAGS.post_image_size,
            interpolation=FLAGS.naive_upscaling,
            split="train",
            real_extension=".JPEG",
        )
    else:
    ...
    if synthetic_dataset is not None:
        ...
        synthetic_datalooper = infiniteloop(synthetic_dataloader)
    else:
        synthetic_datalooper = None
    ...
    for step in range(FLAGS.total_steps):
        ...
        x0, x1, y = get_next_batch(datalooper, synthetic_datalooper)
        ...
        if FLAGS.save_step > 0 and step % FLAGS.save_step == 0:
            ...
            with torch.no_grad():
                ...
                for val_step in range(FLAGS.validation_steps):
                    ...
                    val_t, val_xt, val_ut, _, val_y = (
                        FM.guided_sample_location_and_conditional_flow(
                            val_x0, val_x1, y0=val_y, y1=val_y
                        )
                    )
